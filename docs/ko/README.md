...
* 순위표 오버레이에 Elo 레이팅 기반의 **실시간 예상 iRating 증감 수치 (+/-)** 가 추가되었습니다.
* 내 주변 드라이버들이 어떤 차를 타고 있는지 알 수 있는 **차종(Car Model)** 텍스트 컬럼이 추가되었습니다.

**3. 🚀 7개의 신규 오버레이 추가:**
* **Radar (근접 레이더):** 탑다운 뷰 레이더.
* **Spotter (시각적 스포터):** 사각지대 차량 시각 경고.
* **Incident Warning (사고 경고):** 전방 150m 내 사고 발생 시 스마트 거리 표시 및 배너 경고.
* **Traffic (멀티클래스 트래픽):** 상위 클래스 차량 접근 시 랩타임 차이에 따른 동적 블루 플래그 UI.
* **Flat Map (일자형 맵):** 전체 트랙의 트래픽을 한눈에 보는 리본 맵.
* **Delta Bar (델타 바):** 최적 랩(Optimal) 대비 실시간 가로형 델타 게이지.
* **Pit Helper (피트 헬퍼):** 피트레인 진입 시 속도 제한 및 정차 수리 시간 카운트다운 표시.
